...
/*:
 ...
 With that said, you can use closures as a property of a class or a parameter in a function.
...
//this function has one parameter and is of type Person
func printGuestInfo(guest: Person) {
...
/*:
 ...
 They're very similar aside from the open curly brace is in front of the parameter list and the parameter list and the body of the closure is separated by the `in` keyword
 
 Now lets write a closure and assign it to a variable
 */

let blockOfCodeToPrintGuestInfo = { (guest: Person) in
    print("Name: \(guest.name)", "Age: \(guest.age)")
}
...
/*:
 Lets see how we execute each closure:
 */
...
/*:
 Closures can be used for two things; container of code used as a utility like a validator, or a set a rules to sort by.
 
 The second way of using closures is containing code to be executed later, perhaps after a task is finished (like a network call is finished downloading a list of users from the internet).
 
 ## Closures used as parameters of a function
 
 Lets write a function that adds a new guets to our list of guests but also asks the guest if they have any special request they'd like us to execute.
